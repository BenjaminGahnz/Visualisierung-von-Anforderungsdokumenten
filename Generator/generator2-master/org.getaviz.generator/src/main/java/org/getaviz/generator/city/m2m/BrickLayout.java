...
class BrickLayout {
	...
	static void brickLayout(Long model) {
		List<Record> buildings = connector.executeRead("MATCH (n:City:Model)-[:CONTAINS*]->(b:Building) WHERE ID(n) = " + model + " RETURN b");
		for (Record record : buildings) {
			Node building = record.get("b").asNode();
	...
	private static void separateBuilding(Node building) {
		...
		/*if (connector.executeRead("MATCH (b)-[:HAS]->(n) WHERE ID(b) = " + building.id() + " RETURN n").isEmpty()) {
			return; //TODO: is empty geht nicht muss erst gespeichert werden.
		}*/
		...
		case METHODS_ONLY:
			...
			break;
		default:
			classElements = CityUtils.getChildren(building.id()); //ToDo getChildren anpassen so das es nur bis nächster Section ?
			break;
		}
