...
//			Map<String, Double> properties = null;
			if (entity.hasLabel(Labels.Specification.name())) {
	...
	private static Rectangle calculateMaxArea(Long entity) {
		...
		for (Node child : getChildren(entity)) {
			...
			//! Hier Chapter Logik ergänzen!!! 
			//? könnte es das Labels.Specification.name oder Labels.District.name richtig sein könnte ?
			if (element.hasLabel(Labels.Specification.name()) ||element.hasLabel(Labels.Section.name()))  { //! Hier Chapter Logik ergänzen!!!
				arrangeChildren(child.id());
			}
			sum_width += properties.get(child.id())[0] + config.getBuildingHorizontalGap();
			sum_length += properties.get(child.id())[1] + config.getBuildingHorizontalGap();
		}
		return new Rectangle(0, 0, sum_width, sum_length );
	...
	private static List<Rectangle> sortChildrenAsRectangles(List<Node> children) {
		...
		//! Sorting wurde entfernt, da sonst die innerhalb einer Ebene durcheinander kommen
		//! Nur Containment Beziehungen würden abgebildet werden
		//ToDo: Sollte das unproblematisch funktioneren, dann überlegen ob Sinnvoll Ist  zumindest
		//ToDo: die Sortierung für Spezifikationen beizubehalten um Kompaktere Ordung beizubehalten
	...
	private static void setNewPositionFromNode(Rectangle el, CityKDTreeNode fitNode) {
		...
		connector.executeWrite(String.format(Locale.US,
				"MATCH (n) WHERE ID(n) = %d CREATE (n)-[:HAS]->(p:Position:City {name: 'position', x: %.6f, y: %.6f, z: %.6f})",
				node.id(), x, y, z));

		//*  Kann entfehrnt werden
		List<Record> Labels = connector.executeRead("MATCH (n) WHERE ID(n) = " + node.id() + " RETURN labels(n) as labels");
		for (Record record : Labels) {
			List<Object> labelList = record.get("labels").asList();
			System.out.println("setNewPositionFromNodecalla : "+"Node ID: " + node.id() + ", Labels: " + labelList);
		}
	}
...
//! Problem: Wenn Chapter einzelne Requirements enthält und sonst nur Kapitel, kann es Passieren das sich die Namen überschneiden
	...
	private static void adjustPositions(List<Node> children, double parentX, double parentY, double parentZ) {
		for (Node child : children) {
			...
			double length = properties.get(child.id())[1];
			connector.executeWrite(String.format(Locale.US,
					"MATCH (c),(p) WHERE ID(c) = %d AND ID(p) = %d SET c.width = %.6f, c.length = %.6f, p.x = %.6f, p.y = %.6f, p.z = %.6f",
					child.id(), position.id(), width, length, setX, setY, setZ));
			double height = child.get("height").asDouble();
			
			//! Das wurde gemacht weil die Obere Linke ecke ja die Margin beachten sollte!
			//? --------------------------------------------
			if (entity.hasLabel(Labels.Specification.name()) || entity.hasLabel(Labels.Section.name())) {	
				double newUpperLeftX = setX - width / 2 +  width * 0.05;  
				double newUpperLeftZ = setZ - length / 2 + length * 0.05;
				double newUpperLeftY = setY - height / 2;
				adjustPositions(getChildren(child.id()), newUpperLeftX, newUpperLeftY, newUpperLeftZ);
			}

			//! Bin mir noch nicht sicher ob ich die Beziehung und Position Node
			//!(mit X, y, z als werten ) nicht doch bereits in City2City Hinzufügen! - noch NICHT erledigt<
			//! Beziehungsweise vlt sogar schon in JQA2City
			//ToDo: Dann müsste hier nur noch die x,y,z Werte verändert werden.
			
		}
	}

//? Original: element.hash as hash ORDER BY element.hash 
//? Änderung: ORDER BY ID Weil dann möglicherweise die Hierachische Reihenfolge beibehalten wird

	private static List<Node> getChildren(Long entity) {
		List<Node> children = new ArrayList<>();
		String query = String.format(
		"MATCH (n)-[:CONTAINS]->(c)-[:VISUALIZES]->(element) " +
		"WHERE ID(n) = %d " +
		"RETURN c, element.getavizOrder ORDER BY element.getavizOrder", entity);

		List<Record> childs = connector.executeRead(query);
		if (childs != null && !childs.isEmpty()) {
			for (Record child : childs) {
				children.add(child.get("c").asNode());
			}
		}
		return children;
	}
